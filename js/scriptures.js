...
const Scriptures = (function () {
	...
	const CLASS_BTN_CHAPTER = "btn chapter";
	const CLASS_CHAPTERS = "chapters";
	...
	let retryDelay = 500;
	let volumes;
    let navigatePrevContent = "";
    let navigateNextContent = "";
	...
	*/
	...
	getScriptureCallback = function (chapterHtml) {
	    ...
	    let html = navigatePrevContent + navigateNextContent + chapterHtml;
	...
	navigate = function (bookId, chapter) {
		let previous;
		let next;

		if (previousChapter(bookId, chapter) !==undefined) {
			previous = previousChapter(bookId, chapter);
			let prevBook = previous[0];
			let book = books[prevBook];
			let prevChapter = previous[1];
			let volume = volumeForId(book.parentBookId);

			navigatePrevContent = `<br><a href="#${volume.id}:${book.id}:${prevChapter}" class="button">Previous</a><br>`;
		}

		if (nextChapter(bookId, chapter) !==undefined) {
			next = nextChapter(bookId, chapter);
			let nexBook = next[0];
            let book = books[nexBook];
			let nexChapter = next[1];
			let volume = volumeForId(book.parentBookId);

			navigateNextContent = `<br><a href="#${volume.id}:${book.id}:${nexChapter}" class="button">Next</a><br><br>`;
		}


        navigatePrevContent = htmlDiv({
            id: "prevNav",
            content: navigatePrevContent
        });

        navigateNextContent = htmlDiv({
            id: "nextNav",
            content: navigateNextContent
        });
	...
	navigateBook = function (bookId) {
		...
		if (book.numChapters === 0) {
			...
			for (i = 0; i < book.numChapters; i++) {
				chapter = i + 1;
				gridContent2 += htmlLink({
					classKey: CLASS_BTN_CHAPTER,
					id: bookChapters[i],
					href: `#${volume.id}:${book.id}:${bookChapters[i]}`,
					content: chapter
				});
			}

			gridContent2 = htmlDiv({
				classKey: CLASS_CHAPTERS,
				content: gridContent2
			});

			gridContent = gridContent + gridContent2;
		...
		//console.log(htmlDiv({id: DIV_SCRIPTURES_NAVIGATOR, content: gridContent}));
	...
	navigateChapter = function (bookId, chapter) {
		if (bookId !== undefined) {
            ...
            console.log(navigate(bookId, chapter));

			ajax(encodedScriptureUrlParameters(bookId, chapter), getScriptureCallback, getScriptureFailed, true);
			//document.getElementById("scriptures").innerHTML = "<div>Chapter" + chapter + " " + volume + "</div>";
		}
		console.log("book: " + bookId + ", chapter: " + chapter+ ", url: " + encodedScriptureUrlParameters(bookId, chapter));
	};
